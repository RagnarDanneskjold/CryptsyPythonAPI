...
OPEN_DBS = dict()
#import dbm
#OPEN_DBS = dbm.open('/tmp/cache.dbm','c')
...
def _args_key(function, args, kwargs):
    ...
    key = str(function.__name__ + arguments_pickle)
    return key
        ...
        def function_with_cache(*args, **kwargs):
            try:
                ...
                if key in function._db:
                    rv = function._db[key]
                    if seconds_of_validity is None or _time.time() - rv.timesig < int(seconds_of_validity):
                        #print '... hit cache', int(seconds_of_validity)
                        return rv.data
                    else :
                        #print "... refresh", int(seconds_of_validity)
                        pass
            except Exception:
